...
function View({uploadVideoResponse,setremoveCtegoryVideoResponse }) {
  ...
  const handleCategoryVideo = async (e)=>{
    ...
    setremoveCtegoryVideoResponse(result.data)
  ...
  return (
    ...
    <Row droppable="true" onDragOver={e=>dragOverView(e)} onDrop={e=>handleCategoryVideo(e)}>
      { allVideos?.length>0? allVideos?.map((video,index)=>(
         ...
         <VidioCard displayData={video} setDeleteVideoResponse = {setDeleteVideoResponse}/>
         </Col>
      ))
      :
      <div className="text-danger fw-boder">NO videos are Uploaded Yet!!!</div>
    }
