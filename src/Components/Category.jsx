...
import { Modal,Button, Form, FloatingLabel } from 
...
function Category({removeCtegoryVideoResponse}) {
  ...
  // const handleAddCategory = await addCategoryAPI({categoryName,allVideos:[]})
...
<div className="d-flex justify-content-around">
        ...
        <button onClick={handleShow} className='btn bg-info rounded-circle fs-5 ms-2'><i className="fa-solid fa-plus"></i></button>
      </div>
              ...
              <div className="row mt-2">
                {
                  ...
                  item.allVideos?.map((video,index)=>(
                    <div draggable onDragStart={e=>videoDragStarted(e,video.id,item.id)} key={index} className='col-lg-6'>
                      <VidioCard insideCategory={true} displayData={video}/>
                    </div>
                  ))
                }
              </div>
      ...
      <div className='border rounded border-secondery p-3 '>
        ...
        <Modal.Footer>
          ...
          <Button onClick={handleAddCategory} variant="primary">Add New Category</Button>
        </Modal.Footer>
      </Modal>
